...
// Result for preview (no insertion yet)
...
// Result after confirmation (cards inserted)
...
export type GenerateCardsResponse = GenerateCardsResult | GenerateCardsError;
...
async function checkUserQuota(userId: string, cardCount: number = 10): Promise<{
  ...
  if (!userProfile) {
    ...
    if (createError) {
      console.error("[ai-cards] Failed to create profile:", createError);
      return {
        canGenerate: false,
        isFounderOrAdmin: false,
        error: {
...
export async function generateCardsPreview(
  ...
  const { text, deckId, userId } = input;
 ...
 * Generates cards and inserts them immediately.
 * @deprecated Use generateCardsPreview + confirmAndInsertCards instead
 */
export async function generateCardsFromText(
  input: GenerateCardsInput
): Promise<GenerateCardsResponse> {
  // Generate preview first
  const previewResult = await generateCardsPreview(input);

  if (!previewResult.success) {
    return previewResult;
  }

  // Immediately confirm all cards (legacy behavior)
  const confirmResult = await confirmAndInsertCards({
    deckId: previewResult.deckId,
    userId: input.userId,
    cards: previewResult.cards,
  });
